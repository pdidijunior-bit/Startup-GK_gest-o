...
import { Bell, Clock, ExternalLink, X, Volume2, MessageCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 max-w-md w-full pointer-events-auto animate-in slide-in-from-bottom-5">
      {activeAlarms.map((alarm) => {
        const isWhatsapp = alarm.platform === 'whatsapp' || Boolean(alarm.whatsappCallLink);
        const whatsappHref =
          alarm.whatsappCallLink ||
          (alarm.whatsappNumber ? `https://wa.me/${alarm.whatsappNumber.replace(/[^\d+]/g, '')}` : undefined);

        return (
          <div
            key={alarm.id}
            className={`border-2 shadow-2xl rounded-xl p-4 text-slate-100 flex flex-col gap-3 backdrop-blur-md ${
              isWhatsapp
                ? 'bg-slate-900 border-emerald-500/80 shadow-emerald-500/20'
                : 'bg-slate-900 border-cyan-500/80 shadow-cyan-500/20'
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                >
                  {isWhatsapp ? <MessageCircle className="w-5 h-5" /> : <Bell className="w-5 h-5" />}
                </div>
                <div>
                  <span
                    className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${
                      isWhatsapp ? 'text-emerald-400' : 'text-cyan-400'
                    }`}
                  >
                    <Volume2 className="w-3.5 h-3.5" />
                    {isWhatsapp ? 'Alarme Reunião WhatsApp' : 'Alarme em Tempo Real'}
                  </span>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-300 bg-slate-950/60 px-2.5 py-1.5 rounded-md border border-slate-800">
              <Clock className={`w-3.5 h-3.5 ${isWhatsapp ? 'text-emerald-400' : 'text-cyan-400'}`} />
            ...
            <div className="flex items-center gap-2 pt-1 border-t border-slate-800/80">
              {isWhatsapp && whatsappHref ? (
                <a
                  href={whatsappHref}
                  target="_blank"
                  rel="noreferrer"
                  className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-lg transition-colors shadow shadow-emerald-500/20"
                >
